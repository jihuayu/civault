...
Go 测试包括加密篡改与上下文隔离、配置持久化和并发版本、敏感字段不可回显、Tag OR、动态标签与 Key 禁用、整个请求失败、并发三次防重放、版本固定、JWKS 轮换/缓存/失效、RS256 算法限制、GitHub 邮箱验证和数字 ID 绑定、改邮箱不改变绑定、CSRF、会话过期和管理令牌撤销、密码修改撤销会话、审计失败拒绝发放、通知改期/配置变化/重启/超时/去重以及实际 SQLite 快照恢复。

浏览器测试覆盖初始化、密码登录、Tag 和 Key 创建、版本更新及到期设置、规则覆盖预览与版本、引用 Tag 删除保护、敏感配置保持/替换、两页面修改冲突、管理令牌一次显示与撤销、审计导出、未启用通知提示、移动端和退出登录。
...
- 真实 GitHub OAuth App 登录和真实 Resend 投递，需要在页面配置操作者自己的凭证与已验证域名。本地覆盖了协议和故障模拟，不发送真实邮件。
